    ...
    <div v-if="windowstatus.projectDetailsPage" class="w-screen bg-background p-5 flex flex-col" style="height: calc(100vh - 50px);">
        <div class="editing w-full h-[90%] flex gap-5">
            <div class="preview w-60 h-full flex flex-col gap-2">
                <projectcard class="w-full"/>
                <Button variant="ghost" class="w-full">Export to File</Button>
            </div>
            <div class="discprition w-full flex-grow flex flex-col gap-1">
                <h1 class="text-sm text-gray-400">Unique id: </h1>
                <h2>Project Name</h2>
                <Input />
                <h2>Discprition</h2>
                <Textarea />
                <h2>Grade</h2>
                <ToggleGroup type="single" class="w-full h-20 border border-border flex justify-start px-5">
        ...
        <div class="actions flex gap-2 flex-grow">
            ...
            <Button class="bg-white text-black hover:bg-slate-100 hover:text-black">Edit Questions</Button>
        </div>
    </div>
</template>